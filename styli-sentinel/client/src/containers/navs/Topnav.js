...
const TopNav = ({
  ...
  const [userName, setUserName] = useState(userData?.displayName || userData?.name);
  ...
  useEffect(() => {
    ...
    if (user) setUserName(user?.displayName || user?.name);
  }, [location.pathname]);
  return (
        ...
        <div className="user d-inline-block">
          <UncontrolledDropdown className="dropdown-menu-right">
            <DropdownToggle className="p-0" color="empty">
              <span className="name mr-1">{userName || 'styli sso user'}</span>
            </DropdownToggle>
            <DropdownMenu className="mt-3" right>
