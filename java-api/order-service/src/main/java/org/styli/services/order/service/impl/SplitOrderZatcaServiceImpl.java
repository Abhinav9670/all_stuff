...
import org.styli.services.order.helper.RtoZatcaHelper;
...
import org.styli.services.order.pojo.zatca.*;
...
@Service
public class SplitOrderZatcaServiceImpl {
	...
	private static final Log LOGGER = LogFactory.getLog(SplitOrderZatcaServiceImpl.class);
	...
	public void sendZatcaInvoiceSplitOrdder(SplitSalesOrder splitSalesOrder, Boolean returnFlag, AmastyRmaRequest request) {
		ZatcaConfig zatcaConfig = Constants.orderCredentials.getZatcaConfig();
		SplitSalesOrder orderZatca = splitSalesOrder;
		...
		if (returnFlag == true && request != null) {
			...
			invoiceZatca = splitSalesOrder.getSplitSalesInvoices().stream().findFirst().orElse(null);
			zatcaInvoice = generateZatcaInvoiceOfSplitSalesOrder(invoiceZatca, orderZatca, zatcaConfig, false);
		}
		...
		if (null != zatcaInvoice) {
			...
			try {
				...
				LOGGER.info(
						"Split order Zatca Body : " + orderZatca.getEntityId() + " : " + mapper.writeValueAsString(zatcaInvoice));
	...
	public void sendZatcaCreditMemo(SalesCreditmemo memo, SalesInvoice invoice, SalesOrder order, SplitSalesOrder splitSalesOrder, boolean isRto, Stores store, BigDecimal taxFactor, List<SalesCreditmemoItem> memoItems, BigDecimal totalTaxableAmount) {
		...
		SalesInvoice invoiceZatca = (orderZatca != null ? orderZatca.getSalesInvoices() : splitSalesOrder.getSalesInvoices()).stream().findFirst().orElse(null);
		...
		if(isRto && store != null && taxFactor != null && memoItems != null && totalTaxableAmount != null){
			zatcaInvoice = rtoZatcaHelper.generateRtoZatcaCreditMemo(memoZatca, invoiceZatca, orderZatca, null, zatcaConfig, store, taxFactor, memoItems, totalTaxableAmount);
		}
	...
	public ZatcaInvoice generateZatcaInvoiceOfSplitSalesOrder(SalesInvoice invoice, SplitSalesOrder splitSalesOrder, ZatcaConfig zatcaConfig, Boolean CodRtoZatca ) {
		ZatcaInvoice zatcaInvoice = new ZatcaInvoice();

		Stores store = getStoreById(splitSalesOrder.getStoreId());
		...
		if(CodRtoZatca){
			ID.setEn("RTO-COD-"+splitSalesOrder.getEntityId());
			ID.setAr("RTO-COD-"+splitSalesOrder.getEntityId());
		}else{
		...
		eInvoice.setAccountingCustomerParty(getCustomerAddress(splitSalesOrder.getSalesOrder()));
		...
		Map<String, ProductResponseBody> productsFromMulin = mulinHelper
				.getMulinProductsFromSplitOrder(Collections.singletonList(splitSalesOrder), restTemplate);

		List<SplitSalesOrderItem> salesOrderItems = splitSalesOrder.getSplitSalesOrderItems().stream()
				.filter(e -> e.getProductType().equalsIgnoreCase(OrderConstants.ORDER_ITEM_CONFIGURABLE_TYPE))
				.collect(Collectors.toList());

		List<Integer> salesOrderItemIds = salesOrderItems.stream().map(SplitSalesOrderItem::getItemId)
				.collect(Collectors.toList());

		List<SalesInvoiceItem> salesInvoiceItems = invoice.getSalesInvoiceItem().stream()
				.filter(obj -> salesOrderItemIds.contains(obj.getSplitOrderItemId())).collect(Collectors.toList());
		Integer lineCount = 1;
		for (SalesInvoiceItem invoiceItem : salesInvoiceItems) {

			SplitSalesOrderItem orderItem = salesOrderItems.stream()
					.filter(e -> e.getItemId().equals(invoiceItem.getSplitOrderItemId())).findAny().orElse(null);

			// start
			BigDecimal taxFactor = orderHelper.getExclTaxfactor(orderItem.getTaxPercent());

			BigDecimal giftVoucherAmount = BigDecimal.ZERO;
			if (null != orderItem.getSplitSubSalesOrderItem() && !orderItem.getSplitSubSalesOrderItem().isEmpty()) {
				SplitSubSalesOrderItem subSalesOrderItem = orderItem.getSplitSubSalesOrderItem().stream()
		...
		if (null != splitSalesOrder.getSplitSubSalesOrder() && null !=splitSalesOrder.getSplitSubSalesOrder().getDonationAmount()
			...
			String linePrice = parseNullStr(
					splitSalesOrder.getSplitSubSalesOrder().getDonationAmount().setScale(2, RoundingMode.HALF_UP));
			...
			totalPriceInclTax = totalPriceInclTax.add(splitSalesOrder.getSplitSubSalesOrder().getDonationAmount());
			totalPriceExclTax = totalPriceExclTax.add(splitSalesOrder.getSplitSubSalesOrder().getDonationAmount());
			lineCount++;
		}

		// Import Fee as line Item
		if (null != splitSalesOrder.getImportFee() && splitSalesOrder.getImportFee().compareTo(BigDecimal.ZERO) == 1) {
			InvoiceLine.Item.Name ItemName = new InvoiceLine.Item.Name();
			...
			String linePrice = parseNullStr(splitSalesOrder.getImportFee().setScale(2, RoundingMode.HALF_UP));
			...
			totalPriceInclTax = totalPriceInclTax.add(splitSalesOrder.getImportFee());
			totalPriceExclTax = totalPriceExclTax.add(splitSalesOrder.getImportFee());
			lineCount++;
		}
		...
		if(splitSalesOrder.getSplitSubSalesOrder() != null && splitSalesOrder.getSplitSubSalesOrder().getTotalShukranBurnedValueInCurrency()!= null && splitSalesOrder.getSplitSubSalesOrder().getTotalShukranBurnedValueInCurrency().compareTo(BigDecimal.ZERO)>0){
			payableAmountToSet = payableAmountToSet.subtract(splitSalesOrder.getSplitSubSalesOrder().getTotalShukranBurnedValueInCurrency());
		}
		...
		LOGGER.info("Split order Net Payable Amount: " + netPayableAmount);
		LOGGER.info("Split order Store to Base Rate: " + splitSalesOrder.getStoreToBaseRate());

		BigDecimal basePayableAmount = omsorderentityConverter.getBaseValueDecimal(netPayableAmount,
				splitSalesOrder.getStoreToBaseRate());
		...
		LOGGER.info("Split order Base Payable Amount in Zatca Invoice: " + payableAmount);
	...
	public ZatcaInvoice generateZatcaCreditMemo(SalesCreditmemo memo, SalesInvoice invoice, SalesOrder order, SplitSalesOrder splitSalesOrder,
		...
		List<SalesOrderItem> salesOrderItems = salesOrderItemRepository
				.findSalesOrderItemConfigurableByOrderId(memo.getOrderId());

		List<Integer> saleOrderItemIds = salesOrderItems.stream().map(SalesOrderItem::getItemId)
				.collect(Collectors.toList());
		...
		for (SalesCreditmemoItem creditMemoItem : salesCreditMemoItems) {
			...
			if (null != orderItem) {
				...
				ClassifiedTaxCategory
						.setPercent(orderItem.getTaxPercent().setScale(2, RoundingMode.HALF_UP).toString());
				...
				if (shippingAmount != 0 && "rto".equals(status)) {
					...
					if (orderItem.getTaxPercent() != null) {
						taxPercentage = orderItem.getTaxPercent().doubleValue();
					}
	...
	private ZatcaInvoice getSecondReturnInvoiceData(SplitSalesOrder splitSalesOrder, ZatcaConfig zatcaConfig,
