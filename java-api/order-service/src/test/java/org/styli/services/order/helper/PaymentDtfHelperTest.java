...
public class PaymentDtfHelperTest {
...
//		ReflectionTestUtils.setField(configService, "externalAuthBearerToken", "token,ds,1,1");
	...
	@Test
	public void ypayfortDtfcallSuccessTest() {
		PayforDtfRequest req = new PayforDtfRequest();
		req.setStatus("14");
		req.setResponseCode("14000");
		req.setCommand("PURCHASE");
		req.setRememberMe("YES");
		req.setExpireDate("2310");
		req.setCardNumber("988288898882");
		req.setPaymentOption("VISA");
		salesOrder.setStatus("pending_payment");
		ResponseEntity<String> respo = paymentDtfHelper.payfortDtfcall(salesOrder, req, requestHeader);
		assertNotNull(respo);
	}

	@Test
	public void zpayfortDtfcallHoldTest() {
		PayforDtfRequest req = new PayforDtfRequest();
		req.setStatus("15");
		req.setResponseCode("15777");
		req.setCommand("PURCHASE");
		salesOrder.setStatus("pending_payment");
		ResponseEntity<String> respo = paymentDtfHelper.payfortDtfcall(salesOrder, req, requestHeader);
		assertNotNull(respo);
	...
	@Test
	public void payfortDtfcallTest() {
		PayforDtfRequest req = new PayforDtfRequest();
	...
	@Test
	public void failStatusOnwardsTest() {
		paymentDtfHelper.failStatusOnwards(salesOrder, "");
	}

	@Test
	public void setEstimatedDeliveryTimeForRetryPayment() {
		...
		paymentDtfHelper.releaseStoreCredit(salesOrder, new BigDecimal(10));
		paymentDtfHelper.saveOrderGrid(salesOrder, "");
	}

	@Test
	public void addToQuoteTest() {
		AddToQuoteV4Request req = new AddToQuoteV4Request();
		QuoteUpdateDTOV2 respo = externalQuoteHelper.addToQuote(req, "token", "", "");
		assertNotNull(respo);
		assertEquals(respo.getStatus(), true);
	}

	@Test
	public void saveStoreCreditTest() {
		AddToQuoteV4Request req = new AddToQuoteV4Request();
		externalQuoteHelper.saveStoreCredit(1, new BigDecimal(10), "", "");
	}
	...
	private void setStaticfields() {
		...
		ReflectionTestUtils.setField(paymentDtfHelper, "salesOrderCancelServiceImpl", salesOrderCancelService);
		ReflectionTestUtils.setField(configService, "internalAuthBearerToken", "ffyg,token");
		ReflectionTestUtils.setField(paymentDtfHelper, "externalQuoteHelper", externalQuoteHelper);
		ReflectionTestUtils.setField(paymentDtfHelper, "eASServiceImpl", eASServiceImpl);
	}
