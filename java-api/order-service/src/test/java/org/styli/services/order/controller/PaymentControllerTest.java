...
public class PaymentControllerTest extends AbstractTestNGSpringContextTests {
	...
	@Test
	public void getCfPaymentStatusTest() {
		setStaticfields();
		setSalseOrderData();
		setMockData();
		setorderCred();
		when(salesOrderService.findSalesOrdeForCfPayment()).thenReturn(Arrays.asList(salesOrder));
		ResponseEntity<String> respo = paymentController.getCfPaymentStatus("token", "");
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}

	@Test
	public void cashgramRefundWebhookTest() {
		setStaticfields();
		setSalseOrderData();
		setMockData();
		setorderCred();
//		prepareOrderConstant();
		CashgramWebhookDTO dto = new CashgramWebhookDTO();
		dto.setSignature("sig");
		dto.setCashgramId("1");
		dto.setEvent("event");
		dto.setReason("reason");
		dto.setReferenceId("1");
		dto.setUtr("utr");
		dto.setSignature("VI1gngWYnr6QAoSdNw0Zhca/DQmhp0o47tZRMgAgbFg=");
		...
		when(salesOrderService.findSalesOrdeForCfPayment()).thenReturn(Arrays.asList(salesOrder));
		ResponseEntity<String> respo = paymentController.cashgramRefundWebhook(dto);
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}

	@Test
	public void cashfreeWebhookTest() {
		setStaticfields();
		setSalseOrderData();
		setMockData();
		setorderCred();
		when(salesOrderService.findSalesOrdeForCfPayment()).thenReturn(Arrays.asList(salesOrder));
		ResponseEntity<Boolean> respo = paymentController
				.cashfreeWebhook("{data={payment={payment_status=approved,cf_payment_id=1},order={order_id=1}}}", "");
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}

	@Test
	public void refundTabbyPaymentTest() {
		setStaticfields();
		setorderCred();
		setSalseOrderData();
		setMockData();
		TabbyRefundDTO dto = new TabbyRefundDTO();
		dto.setOrderId(1);
		...
		SalesCreditmemo mm = new SalesCreditmemo();
		mm.setAmstorecreditAmount(new BigDecimal(1));
		when(refundHelper.createCreditMemo(any(), anyString(), any(), any(), anyString(), any(), anyBoolean(),
				any(), any(), any(), any(), any())).thenReturn(mm);
		when(paymentDtfRefundHelper.getCancelAmount(any(), any(), any())).thenReturn(new BigDecimal(10));
		RefundPaymentRespone re = new RefundPaymentRespone();
		re.setStatusMsg("success");

		when(paymentDtfRefundHelper.payfortRefundcall(any(), any(), any(), anyString())).thenReturn(re);
		ResponseEntity<String> repo = paymentController.refundTabbyPayment(dto, "token");
		assertEquals(repo.getStatusCode(), HttpStatus.OK);
	}
	...
	public void getWebhookTest() {
		...
		dto.setId("1");
		BNPLOrderUpdateResponse rr = new BNPLOrderUpdateResponse();
		rr.setPaymentSuccess(true);
		rr.setOrderEntityId(1);
		when(tabbyPaymentService.updatePayment(any(), anyBoolean(), anyString())).thenReturn(rr);
		when(tabbyPaymentService.updatePaymentStatus(any(), anyString())).thenReturn(rr);
		ResponseEntity<Boolean> respo = paymentController.getWebhook("{id=1,status=success}", "");
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}

	@Test
	public void tamaraWebhookTest() {
		setStaticfields();
		setSalseOrderData();
		setMockData();
		setorderCred();
		...
		rr.setPaymentSuccess(true);
		rr.setOrderEntityId(1);
		when(httpServletRequest.getHeader(anyString())).thenReturn("auth");
		when(tabbyPaymentService.updatePayment(any(), anyBoolean(), anyString())).thenReturn(rr);
		when(tabbyPaymentService.updatePaymentStatus(any(), anyString())).thenReturn(rr);
		ResponseEntity<Boolean> respo = paymentController.tamaraWebhook("{order_id=1,event_type=order_expired}", "");
		// "{data={payment={payment_status=approved,cf_payment_id=1},order={order_id=1}}}");
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}

	@Test
	public void updateInventoryForExpiredOrderTest() {
		setStaticfields();
		setSalseOrderData();
		setMockData();
		setorderCred();
		...
		when(salesOrderService.findPendingProxyOrders()).thenReturn(Arrays.asList(order));
		ResponseEntity<String> respo = paymentController.updateInventoryForExpiredOrder("token", "");
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}

	@Test
	public void getTabbyPaymentStatusTest() {
		setStaticfields();
		setSalseOrderData();
		setMockData();
		setorderCred();
		...
		when(tabbyPaymentService.updatePayment(any(), anyBoolean(), anyString())).thenReturn(rr);
		when(tabbyPaymentService.updatePaymentStatus(any(), anyString())).thenReturn(rr);
		when(tabbyPaymentService.getPaymentUpdates("")).thenReturn(Arrays.asList(rr));

		when(salesOrderService.findPendingProxyOrders()).thenReturn(Arrays.asList(order));
		ResponseEntity<String> respo = paymentController.getTabbyPaymentStatus("token", "");
		assertEquals(respo.getStatusCode(), HttpStatus.OK);
	}
