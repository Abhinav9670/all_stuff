...
//		ReflectionTestUtils.setField(configService, "externalAuthBearerToken", "token,ds,1,1");
	...
	@Test
	public void updateOrderStatusHistoryTest() {
		UpdateOrderRequest req = new UpdateOrderRequest();
		req.setMessage("msg");
		orderHelper.updateOrderStatusHistory(req, salesOrder);
		orderHelper.createOrderStatusHistory(salesOrder, "");
	}

	@Test
	public void createOrderPaymentTest() {
		QuoteDTO qoute = setQuoteData();
		orderHelper.createOrderPayment(qoute, salesOrder, "");
	}

	@Test
	public void checkFirstOrderByCustomerTest() {
		orderHelper.checkFirstOrderByCustomer("");
		orderHelper.checkFirstCreateOrder("");
	}

	@Test
	public void blockInventoryTest() {
		orderHelper.blockInventory(salesOrder);
		orderHelper.returnInventoryWmsRestCall("1");
	}

	@Test
	public void sendCancelOrderSmsAndEMailTest() {
		UpdateOrderRequest req = new UpdateOrderRequest();
		req.setMessage("msg");
		orderHelper.sendCancelOrderSmsAndEMail(salesOrder, "10");
	}

	private QuoteDTO setQuoteData() {
...
//		ReflectionTestUtils.setField(orderpushHelper, "eASServiceImpl",eASServiceImpl);
	...
	void setMockData() {
		...
		ResponseEntity<Object> postoresponse = new ResponseEntity<>(HttpStatus.OK).ok("");
		Mockito.when(restTemplate.exchange(Mockito.anyString(), Mockito.eq(HttpMethod.POST),
				Mockito.any(HttpEntity.class), Mockito.eq(Object.class))).thenReturn(postoresponse);

		CustomerUpdateProfileResponse cpsbody = new CustomerUpdateProfileResponse();
		CustomerProfileResponse cus = new CustomerProfileResponse();
		Customer customer = new Customer();
		customer.setCustomerId(1);
		cus.setCustomer(customer);
		cpsbody.setResponse(cus);
		ResponseEntity<CustomerUpdateProfileResponse> eresponse = new ResponseEntity<>(HttpStatus.OK).ok(cpsbody);
		...
		when(salesOrderRepository.findByCustomerEmail(anyString())).thenReturn(Arrays.asList(salesOrder));
