...
public class CoreConfigDataServiceImplTest {
...
//		ReflectionTestUtils.setField(configService, "externalAuthBearerToken", "token,ds,1,1");
	...
	@Test
	public void testAll() throws NotFoundException {
		...
		when(coreConfigDataRepository.findByPathAndScopeId(anyString(), eq(1))).thenReturn(null);
		coreConfigDataService.getRMAThresholdInHours(stores);
		when(coreConfigDataRepository.findByPathAndScopeId(anyString(), eq(1))).thenReturn(data);
		coreConfigDataService.getStoreShipmentChargesThreshold(stores);
		coreConfigDataService.getStoreShipmentCharges(stores);
		coreConfigDataService.getCodCharges(stores);
		coreConfigDataService.getTaxPercentage(stores);
		coreConfigDataService.getCustomDutiesPercentage(stores);
		coreConfigDataService.getImportFeePercentage(stores);
		coreConfigDataService.getMinimumDutiesAmount(stores);
	}
