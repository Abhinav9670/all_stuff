...
public class PrepaidRefundHelperTest {
	...
	@Mock
	SalesCreditmemoRepository creditmemoRepository;
	@Mock
...
//		ReflectionTestUtils.setField(configService, "externalAuthBearerToken", "token,ds,1,1");
	...
	@Test
	public void prepaidRefundCallTest() {
		...
		Set<SalesOrderPayment> payitem2 = new HashSet<>();
		payitem.add(pay);
		payitem2.add(pay1);
		salesOrder.setSalesOrderPayment(payitem);
		when(tabbyHelper.refundPayment(any(), anyString())).thenReturn(response);
		when(paymentDtfRefundHelper.payfortRefundcall(any(), any(), any(), anyString())).thenReturn(response);
		...
		when(paymentDtfRefundHelper.cancelPercentageCalculation(any(), any(), any(), any(), anyBoolean(), anyString(),
				any())).thenReturn(new BigDecimal(10));
		prepaidRefundHelper.prepaidRefundCall(salesOrder);
		salesOrder.setSalesOrderPayment(payitem2);
		prepaidRefundHelper.prepaidRefundCall(salesOrder);
		when(paymentDtfRefundHelper.cancelPercentageCalculation(any(), any(), any(), any(), anyBoolean(), anyString(),
				any())).thenReturn(new BigDecimal(0));
		prepaidRefundHelper.prepaidRefundCall(salesOrder);
...
//		salesInvoiceItem.set
	...
	private void setStaticfields() {
		...
		ReflectionTestUtils.setField(prepaidRefundHelper, "eASServiceImpl", eASServiceImpl);
//    ReflectionTestUtils.setField(tabbyPaymentService, "refundHelper", refundHelper);
		...
		ReflectionTestUtils.setField(prepaidRefundHelper, "paymentUtility", paymentUtility);
//		ReflectionTestUtils.setField(prepaidRefundHelper, "paymentDtfHelper", paymentDtfHelper);
	}
