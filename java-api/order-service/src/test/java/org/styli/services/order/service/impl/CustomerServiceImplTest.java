...
public class CustomerServiceImplTest {
	...
	@Mock
	org.styli.services.order.repository.Customer.CustomerEntityRepository customerEntityRepository;
	@Mock
	org.styli.services.order.repository.Customer.CustomerAddressEntityRepository customerAddressEntityRepository;
	@Mock
...
//		ReflectionTestUtils.setField(configService, "externalAuthBearerToken", "token,ds,1,1");
  ...
  public void getCustomerDetailsTest() {
	  ...
	  CustomerAddressEntity customerAddressEntity = new CustomerAddressEntity();
		...
		attrMap.put(1,"area");
		attrMap.put(2,"");
		when(staticComponents.getAttrMap()).thenReturn(attrMap);
		CustomerUpdateProfileResponse respo = customerService.getCustomerDetails(1,requestHeader);
		assertEquals(respo.isStatus(), true);
  }

	@Test
	public void adeductStoreCreditV2failTest() throws NotFoundException {
		QuoteDTO qoute = setQuoteData();
		Stores stores = new Stores();
		stores.setCurrencyConversionRate(new BigDecimal(1));
		stores.setStoreId("1");
		customerService.deductStoreCreditV2(qoute, salesOrder, stores, new BigDecimal(10));
		customerService.getCustomerStoreCredit(1);
		verify(amastyStoreCreditRepository, atLeast(1)).findByCustomerId(anyInt());
	}

	@Test
	public void deductStoreCreditV2Test() throws NotFoundException {
		Stores stores = new Stores();
		stores.setCurrencyConversionRate(new BigDecimal(1));
		stores.setStoreId("1");
