...
public class ConsulComponent {
  ...
  public void init() throws NotFoundException {
    ...
    Consul client =
        Consul.builder().withUrl("http://" + consulIpAddress + ":" + consulPort)
	...
	if ("IN".equals(region)) {
		gcpStorage.cacheAdrsmprJson(region.toLowerCase());
	} else {
    ...
    cacheInvUrl(Constants.CONSUL_CATALOG_KEYS, env, kvClient);
    ...
    cacheloginCredentials(Constants.CONSUL_CUSTOMER_CREDENTIALS_KEY, kvClient,env);
    authcache(Constants.AUTH_URL_KEY, env, kvClient, true);
    quoteCache(Constants.CONSUL_QUOTE_SERVICE_KEY, env, kvClient);
    
    cacheAppConfig(Constants.CONSUL_APPCONFIG_KEYS, kvClient, env);
   	cacheBaseConfig(Constants.CONSUL_BASE_CONFIG_KEYS, kvClient, env);
		...
		cache.addListener(newValues -> {
			for (com.orbitz.consul.model.kv.Value value : newValues.values()) {
				if (value.getKey().equals("addressMapper_" + countryCode + "_" + env)) {
					...
					LOGGER.info("decodedValue:" + countryCode + " " + decodedValue);
  ...
  public static void cacheloginCredentials(String key, KeyValueClient kvClient, String env) {
      String cacheKey = key+"_"+env ;
      KVCache cache = KVCache.newCache(kvClient, cacheKey);
      cache.addListener(newValues -> {
          for (com.orbitz.consul.model.kv.Value value : newValues.values()) {
              if (value.getKey().equals(cacheKey)) {
              	...
              	String decodedValue = value.getValueAsString().get();
              	LOGGER.info("login credentials:"+decodedValue);
              	Constants.setLoginCredentials(decodedValue);
	...
	public static void cacheInvUrl(String key, String env, KeyValueClient kvClient) {
		...
		cache.addListener(newValues -> {
			for (com.orbitz.consul.model.kv.Value value : newValues.values()) {
				if (value.getKey().equals(key + "_" + env)) {
					...
					Constants.setCatalogConsulValues(String.valueOf(decodedValue));
				}
			}
		});
		cache.start();
	 ...
	 public static void authcache(String key, String env, KeyValueClient kvClient, boolean isFromPromoUrl) {
		...
		public static void quoteCache(String key, String env, KeyValueClient kvClient) {
			String cacheKey = key + "_" + env;
			KVCache cache = KVCache.newCache(kvClient, cacheKey);
			cache.addListener(newValues -> {
				for (com.orbitz.consul.model.kv.Value value : newValues.values()) {
					if (value.getKey().equals(cacheKey)) {
						...
						String decodedValue = value.getValueAsString().get();
						LOGGER.info("quote consul:" + decodedValue);
						Constants.setFreeShipping(decodedValue);
	            ...
	            for (com.orbitz.consul.model.kv.Value value : newValues.values()) {
	                if (value.getKey().equals(cacheKey)) {
	                    ...
	                    try {
	                         ...
	                         Constants.setConsulConfigResponse(storeConfigResponseFromConsul);
	 ...
	 public static void cacheBaseConfig(String key, KeyValueClient kvClient, String env) {
			String cacheKey = key + "_" + env;
			KVCache cache = KVCache.newCache(kvClient, cacheKey);
			cache.addListener(newValues -> {
				for (com.orbitz.consul.model.kv.Value value : newValues.values()) {
					if (value.getKey().equals(cacheKey)) {
						...
						String decodedValue = value.getValueAsString().get();
						LOGGER.info("base config :" + decodedValue);
						try {
							Gson g = new Gson();
							BaseConfig baseConfigResponse = g.fromJson(decodedValue, BaseConfig.class);
							Constants.setConsulBaseConfigResponse(baseConfigResponse);
						} catch (Exception exception) {
							LOGGER.error("error occurred during parsing base config: " + exception.getMessage());
						}
	 ...
	 private static void setConsul(Consul clientObaject) {
			
		  	client = clientObaject;
			}
