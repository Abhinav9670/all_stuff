...
import org.styli.services.customer.jwt.security.jwtsecurity.security.JwtAuthenticationTokenFilter;
import org.styli.services.customer.jwt.security.jwtsecurity.security.JwtSuccessHandler;

@EnableGlobalMethodSecurity(prePostEnabled = true)
@EnableWebSecurity
@Configuration
public class JwtSecurityConfig extends WebSecurityConfigurerAdapter {
  private static final Log LOGGER = LogFactory.getLog(JwtSecurityConfig.class);
  @Autowired
  ...
  protected void configure(HttpSecurity http) throws Exception {
    ...
    http.csrf().disable().authorizeRequests().antMatchers(HttpMethod.OPTIONS, "/**").permitAll()
        .antMatchers("**/rest/customer/auth/**").authenticated().and().exceptionHandling().authenticationEntryPoint(entryPoint)
  ...
  public CorsConfigurationSource corsConfigurationSource() {
    ...
    if (env.contains("dev") || env.contains("qa") || env.contains("qa01") || env.contains("uat")) {
      allowedOrigins = corsUrls + ",http://local.stylifashion.com:3000,https://local.stylifashion.com:3000,http://localhost:3002";
    } else {
